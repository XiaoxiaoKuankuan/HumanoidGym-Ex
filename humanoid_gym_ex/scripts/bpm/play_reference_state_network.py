...
默认路径：
    模型：deploy/reference_state_keypoint_model.pt
    验证数据：BPM_dance/bpm_phase_state_dataset_keypoint
    输出目录：reference_state_eval

运行指令：
    查询单个当前相位参考量：
        python humanoid_gym_ex/scripts/bpm/play_reference_state_network.py \
            --model /abs/path/to/reference_state_keypoint_model.pt \
            --bpm 90 --phase 1.57
...
DEFAULT_MODEL_PATH = PROJECT_ROOT / "deploy" / "reference_state_keypoint_model.pt"

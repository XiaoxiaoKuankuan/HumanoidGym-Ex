...
输入输出：
    ...
    默认模型：deploy/reference_state_keypoint_model.pt
    默认 ONNX：deploy/reference_state_keypoint_model.onnx

运行指令：
    推荐在当前仓库根目录运行，并显式指定外部数据目录：
        python humanoid_gym_ex/scripts/bpm/train_reference_state_network.py \
            --data-dir /abs/path/to/bpm_phase_state_dataset_keypoint \
    ...
    CPU 快速测试：
        python humanoid_gym_ex/scripts/bpm/train_reference_state_network.py \
            --data-dir /abs/path/to/bpm_phase_state_dataset_keypoint \
            --output /tmp/reference_state_keypoint_model.pt \
            --epochs 1 --device cpu
...
try:
    ...
    PROJECT_ROOT = Path(LEGGED_GYM_ROOT_DIR)
...
DEFAULT_OUTPUT_PATH = PROJECT_ROOT / "deploy" / "reference_state_keypoint_model.pt"

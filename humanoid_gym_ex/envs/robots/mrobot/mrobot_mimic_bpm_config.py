"""BPM/music mimic config entry point."""
...
class MrobotMimicBPMCfg(MrobotMimicCommonCfg):
    """BPM/music mimic task config.

    This is a sibling of ``MrobotMimicDanceCfg``: both inherit the shared
    ...
    class motion(MrobotMimicCommonCfg.motion):
        reference_source = "bpm"
...
class MrobotMimicBPMCfgPPO(MrobotMimicCommonCfgPPO):
    ...
    class runner(MrobotMimicCommonCfgPPO.runner):
        ...
        save_config = "mrobot_mimic_bpm_config.py"


# Backward-compatible aliases. These let old code keep using the short class
# names while new import paths make the BPM task explicit.
MrobotMimicCfg = MrobotMimicBPMCfg
MrobotMimicCfgPPO = MrobotMimicBPMCfgPPO
